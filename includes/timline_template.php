...
if (!class_exists('Timeline_Template')) {
    ...
    class Timeline_Template {
        ...
        public function __construct() {
            // register actions
			add_action('init', array(&$this, 'timeline_register_shortcode'));
		...
		function ctl_custom_read_more() {
		   ...
		   function ctl_custom_excerpt_more($more) {
			  global $post;
			   $ctl_options_arr = get_option('timeline_options');
				if ($post->post_type == 'timeline' && !is_single() ){
					 if( $ctl_options_arr['display_readmore']=='yes'){
							 return '..<a class="read_more ctl_read_more" href="'. get_permalink($post->ID) . '">'. __('Read More', 'timeline') .'</a>';
					   }  
				 }else{
				  	return $more;
				 }
        ...
        function timeline_register_shortcode() {
            add_shortcode('timeline', array(&$this, 'timeline_view'));
		 }
	    function timeline_view($atts, $content = null) {
		  ...
		  'post_status' => array( 'publish', 'future' ),
            ...
            if ($ctl_loop->have_posts()){
                ...
                while ($ctl_loop->have_posts()) : $ctl_loop->the_post();
					...
					$posted_date=get_the_date(__('M d','timeline'));
                    ...
                    if ($i % 2 == 0) {
						...
						$ctl_html .='<div class="events"><h4 class="events-heading">' . get_the_title() . '</h4>';
						 ...
						 $ctl_html   .= '<a href="'. get_permalink($post->ID) . '"><div class="ctl_info event-description '.$cont_size_cls.'">';
						if(isset($ctl_thumb_url)&& !empty($ctl_thumb_url)){
					   if($cont_size_cls=="full"){
								$ctl_html.='<div class="full-width"><img  width="100%" class="events-object" src="'.$ctl_thumb_url.'"></div></a>';
							}else{
							$ctl_html.='<div class="pull-left"><img  class="events-object left_small" src="'.$ctl_thumb_url.'"></div></a>';
							}
						}
					
						if(!empty($post_content)){
						$ctl_html.='<div class="events-body"><a href="'. get_permalink($post->ID) . '"><p>' . $post_content. '</p></a></div>';
						 }
    ...
    <div class="timeline">';
		if(isset($user_avatar[0])&& !empty($user_avatar[0])){
         ...
         $output .='<div class="row">
		  <div class="col-md-12">
			  <div class="timeline timeline_cont"><dl>';
		$output.=$ctl_html;
		$output.='</dl></div>
			</div>
		</div>
      ...
      ================================================== -->';
		...
		function ctl_custom_style(){
            ...
            if ($background_type == 'on') {
                if ($bg_img) {
                    $styles.='.timeline{background-image:url("' . $bg_img . '")};';
                }
                $styles.='.timeline{background-color:' . $bg_color . ';}';
            }
				$styles .=' 
			...
			$styles .='.timeline h1.timeline-main-title{font-family:' .$events_body_f . ';}';
			$styles .='.timeline .timeline-main-title{text-align:' .$title_alignment . ';}';
		...
		*/
        ...
        function ctl_load_scripts_styles() {
			...
			wp_register_style('ctl_styles', TIMELINE_PLUGIN_URL . 'css/ctl_styles.css',null, null,'all' );
