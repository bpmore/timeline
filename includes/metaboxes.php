...
add_action( 'admin_menu', 'weothemes_metabox_add', 10 ); // Triggers timeline_metabox_create()
...
// Woo Metabox Options
$ct_metaboxes = array();
if( get_post_type() == 'timeline' || !get_post_type() ){
	$ptype=get_post_type();

//        $ct_metaboxes[] = array('name' => 'text',
//            'label' => __('text', 'timeline'),
//            'type' => 'text',
//            'desc' => __('Upload an image or enter an URL.', 'timeline'));

        $ct_metaboxes[] = array('name' => 'image_container_type',
            'std' => __('default', 'timeline'),
            'label' => __('Image Size', 'timeline'),
            'type' => 'select',
            'desc' => __('You can update these size from admin plugin option panel', 'timeline'),
            'options' => array(
                 'large' => 'Full',
               // 'Medium' => 'Medium',
                'small' => 'small'));
        ...
        if (function_exists('get_post_types')) {
            ...
            foreach ($custom_post_list as $type) {
                ...
                $settings = array(
                    ...
                    'title' => sprintf(__('%s Custom Settings', 'timeline'), $theme_name),
          ...
          add_meta_box( 'weo-settings', sprintf( __( '%s Custom Settings', 'timeline' ), $theme_name ), 'ct_metabox_create', 'post', 'normal' );
          add_meta_box( 'weo-settings', sprintf( __( '%s Custom Settings', 'timeline' ), $theme_name ), 'ct_metabox_create', 'page', 'normal' );
          } */
    }
}

// End timeline_metabox_add()
...
// Array sanity check.
    ...
    if ($callback['id'] == 'weo-meta-settings') {
        ...
        if ($display_general_fields) {
           // $output .= '<li class="wf-tab-general"><a href="#wf-tab-general">' . __('Extra Settings', 'timeline') . '</a></li>' . "\n";
        }
    ...
    if ($display_general_fields) {
        $output .= timeline_metabox_create_fields($ct_metaboxes, $callback, 'general');
    }
    // Allow themes/plugins to add tabs to WooFramework custom fields.
...
// End timeline_metabox_create()

/**
 * timeline_metabox_create_fields()
 *
 ...
 */
function timeline_metabox_create_fields ( $metaboxes, $callback, $token = 'Extra Meta' ) {
global $post;
		...
		if( isset( $ct_metabox['type'] ) && ( in_array( $ct_metabox['type'], $ct_metabox) ) ) {
			$ct_metaboxvalue = get_post_meta($post->ID,$ct_name,true);
				  ...
				  elseif( $ct_metabox['type'] == 'upload' )
        	    {
        	    	...
        	    	if ( function_exists( 'timeline_medialibrary_uploader' ) ) {
	    	            ...
	    	            $output .= "\t\t".'<td class="ct_metabox_fields">'. timeline_medialibrary_uploader( $ct_metabox['name'], $_value, 'postmeta', $ct_metabox['desc'], $post->ID );
	    	            $output .= '</td>'."\n";
	    	            $output .= "\t".'</tr>'."\n";
    ...
    return $output;
} // End timeline_metabox_create_fields()
